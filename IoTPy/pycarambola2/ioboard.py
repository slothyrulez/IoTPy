...
#from IoTPy.pycarambola2.pinout import CARAMBOLA2_DEVBOARD_PINOUT
...
class Carambola2IoBoard(IoBoard, GPIOProducer, SPIProducer, I2CProducer):
    ...
    def __init__(self, pinout=None):
        ## TODO: Use the core pinout functionality
        ##self.pinout = pinout
        self.pinout = self.gpio_names
    ...
    def GPIO(self, name, *args, **kwargs):
        if name not in self.pinout:
            raise ValueError("Invalid GPIO name %s. Must be one of %s." % (name, ", ".join(self.pinout)))
    ...
    def SPI(self, name, clock=1000000, mode=SPI.MODE_0, *args, **kwargs):
        ...
        if name not in self.pinout:
            raise ValueError("Invalid SPI CS name %s. Must be one of %s." % (name, ", ".join(self.pinout)))
